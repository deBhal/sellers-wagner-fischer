...
describe( 'distances', function() {
	...
	it('should return 0s *at the end of* where the string fits', function() {
		assert.equal(0, last(distances('a mat', 'mat')));
	});

	it('should return 0s *at the end of* every place the string fits', function() {
		assert.equal(0, distances('bat bit bat bot', 'bat')[2]);
		assert.equal(0, distances('bat bit bat bot', 'bat')[10]);
	});
